(ns newrelic.main
  "Kotodama WASM entrypoint for the Newrelic clean-room actor (L4) — Clojure port.
  ...
  facts (`newrelic.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "newrelic")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"newrelic.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "newrelic-compat" :tier tier :entities entities} 200])
...
(defn start! [] :newrelic-compat/ready)
